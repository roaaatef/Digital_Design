...
`uvm_info("run_phase", $sformatf("expected_out value : %b , leds_expected : %b", seq_item_sb.expected_out, seq_item_sb.leds_expected), UVM_LOW);
            ...
            `uvm_error("run_phase", $sformatf("Comparsion failed, Transaction received by the DUT:%s  While the expected out:0b%0b & leds_expected : %b", seq_item_sb.convert2string(), seq_item_sb.expected_out,seq_item_sb.leds_expected));
            error_count++;
            end
